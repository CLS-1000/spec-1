...
# Canonical case-id format generated by spec1_engine.core.ids.case_id()
_CASE_ID_RE = re.compile(r"^case-([0-9a-f]{12})$")


def _validate_case_id(case_id: str) -> None:
    """Raise ValueError if *case_id* does not match the expected format.
...
def _case_file_path(case_id: str) -> Path:
    """Return the canonical case-file path for a validated case ID."""
    _validate_case_id(case_id)
    match = _CASE_ID_RE.fullmatch(case_id)
    assert match is not None
    canonical_case_id = f"case-{int(match.group(1), 16):012x}"
